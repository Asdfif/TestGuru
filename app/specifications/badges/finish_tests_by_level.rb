module Badges
  class FinishTestsByLevel < AbstractRuleSpecification
    ...
    def satisfies?
      user_badges_count = @test_passage.user.badges.where(rule_type: "finish_tests_by_level", rule_value: @value).count
      ...
      when nil
        ...
        array_success_tests = users_success_tests - Test.where.not(level: @value).to_a
        hash_success_tests = array_success_tests.map { |test| [test, array_success_tests.count(test)] }.uniq!.to_h
        Test.where(level: @value).to_a.each do |test| 
          result = false if hash_success_tests[test].nil?
        end
      end
